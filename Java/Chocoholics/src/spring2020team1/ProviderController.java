...
/**
 * Provider controller is a control class which is responsible accessing the
 * provider directory and validating the provider
 *
 *
 * <p>
 * ProviderController can be invoked by an operator or manager at any time
 * </p>
 *
...
public class ProviderController {
	 ...
	 *
	 * @param int integer for the member number
	 * @return the date and time submitted
	 */
	 ...
	 *
	 * @param int the int is the service code
	 * @return the date and time submitted
	 */
	...
	/**
	 * . will check to see if member is a current member and will return the status
	 * of the member
	 *
	 * @param int the members id number
	 * @return string the status of the member if the member exists
	 */
		...
		for (i = 0; i < userList.memberList.size(); ++i) {
			if (userList.memberList.at(i).idNumber == member_num) {
				System.out.println("Member has been found \n" + "Member status: "
						+ userList.memberList.at(i).getMembershipStatus());
				return;
			}
		}
	 ...
	 *
	 * @param int the code for the service to be looked up
	 * @return the description of the service so the provider can validate it is the
	 *         correct service that was provided
	 */
